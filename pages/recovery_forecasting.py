...
#     df_class['is_recovering_next'] = (
#         df_class['recovery_time_sec'].shift(-1) > 0
#     ).astype(int)
#     for feat in features:
#         for lag in range(1, 4):
...
#     cols = [c for c in _scaler.feature_names_in_]
...
# # === Streamlit App ===
# def main():
#     # st.experimental_set_page_title("Recovery Forecast")
#     # st.set_page_config(page_title='Recovery Forecast Dashboard', layout='wide')
#     st.title('📈 Recovery Time Forecast Dashboard')
#     st.markdown('Combine continuous forecasting and binary classification to monitor system recovery.')
...
#     # Classification: Will Recovery Start?
#     st.subheader('▶️ Classification: Will Recovery Start Next?')
#     st.write('The model predicts whether the system will begin recovery in the next time step based on recent metrics.')
#     st.write('Anticipating recovery start helps you prepare resources or trigger alerts before performance impacts occur.')
#     cls = res['cls_report']
#     acc = cls['accuracy']
#     prec = cls['macro avg']['precision']
#     rec = cls['macro avg']['recall']
#     f1 = cls['macro avg']['f1-score']
...
#     # Regression Metrics
#     st.subheader('▶️ Regression Metrics')
...
#         'Select forecast horizon (minutes)',
#         min_value=1, max_value=30, value=5, step=1
#     )
#     forecast_df = simulate_forecast(
#         res['df_forecast'], res['scaler'], res['model'], res['features'], horizon
#     )
#     st.line_chart(forecast_df.set_index('minute'))
#     st.download_button(
#         label='Download Forecast CSV',
#         data=forecast_df.to_csv(index=False),
#         file_name=f'forecast_{horizon}min.csv',
#         mime='text/csv'
#     )
